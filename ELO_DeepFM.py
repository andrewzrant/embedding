...
class DeepFM(BaseEstimator, TransformerMixin):
    def __init__(self, feature_size, field_size,
                 ...
                 epoch=30, batch_size=256,
                 learning_rate=0.001, optimizer_type="adam",
                 batch_norm=1, batch_norm_decay=0.995,
                 verbose=False, random_seed=2016,
                 use_fm=True, use_deep=True,
                 loss_type="mse", eval_metric=rmse,
                 l2_reg=0.1, greater_is_better=False):
            ...
            if self.verbose > 0 and epoch % self.verbose == 0:
                if has_valid:
                    print("[%d/%d] train-result=%.4f, valid-result=%.4f [%.1f s]"
                        % (epoch + 1, self.epoch, train_result, valid_result, time() - t1))
                else:
    ...
    def predict(self, Xi, Xv):
        ...
        while len(Xi_batch) > 0:
            ...
            feed_dict = {self.feat_index: Xi_batch,
                         ...
                         self.label: y_batch,
                         self.dropout_keep_fm: [0.4] * len(self.dropout_fm),
                         self.dropout_keep_deep: [0.4] * len(self.dropout_deep),
